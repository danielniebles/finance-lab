...
- **Telegram:** the webhook at `/api/telegram` calls `runAgentTurn()` (buffered, no streaming), sends text and proposal messages with inline keyboard buttons; button taps call `resolveProposal()` directly. Telegram is also the only channel that currently accepts **image input**: a `message.photo` update (e.g. a credit-card statement screenshot) is downloaded (`getFile` → `downloadFile`) and passed to the model as an Anthropic image content block on the live incoming message, via `runImageTurnAndDeliverToTelegram()`. The image itself is never persisted — `ChatMessage` stores a short text placeholder (`"📸 [foto de tarjeta recibida]"`), matching how text messages are stored. A "📸 Leyendo el pantallazo…" echo is sent before the turn runs, mirroring the shortcut-ingest text echo (ADR-029). This is Part 1 of the card-screenshot batch-ingestion feature (see `.scratch/card-screenshot-image-ingestion.md`); the batch-proposal tool and its rendering are a separate follow-up pass.
...
| `get_counterparty_rules()` | `getCounterpartyRules()` | All `CounterpartyRule` rows (matchType/value, direction, → category + wallet, autoRecord, recurring, matchCount) — used to resolve a ruleId before an update/delete proposal, or to check for an existing rule before creating one (ADR-032). The model does **not** need to call this before `propose_add_transaction` — rule matching there is automatic (ADR-033). |
...
| `propose_add_transaction(amount, date?, appCategoryName?, wallet?, note?, counterpartyAccount?, counterpartyMerchant?, counterpartySender?, direction?)` | `createTransaction()` **or** the auto-record path | Add a single bot-captured expense/income record (ADR-030). Signed amount: negative = expense, positive = income. Call `get_categories` first to guess `appCategoryName` — category is **never** a blocking field; an unresolved/omitted guess falls back to a default and is always editable directly on the card (ADR-031). `wallet` is a free-text account label, not a `SavingsAccount` — no balance coupling. Also extract the counterparty (destination account/merchant/sender) when the message names one — if it matches a `CounterpartyRule` with `autoRecord` enabled and the amount/date parse confidently, the transaction is recorded **immediately** (using the rule's own category/wallet, not your guess) instead of producing a card, and the user is notified on Telegram instead (ADR-033). You do not need to call `get_counterparty_rules` for this to happen. |
| `propose_add_transactions_batch(items: {vendor, amount, date?, scratched?}[], cardLabel?)` | one `createTransaction()` per **included** item | Add multiple expense transactions extracted from a credit-card statement screenshot (ADR-034). `amount` per item is always a positive magnitude — the resolver/executor negates it, since card purchases are always expenses. `scratched: true` (best-effort, model's own vision judgment) pre-excludes a row (`included = !scratched`); the user can re-include/exclude any row on the card. Category per item comes from a matching `CounterpartyRule` (by vendor, direction always EXPENSE) or, failing that, the same no-name fallback guess `propose_add_transaction`'s card uses — never the rule's `wallet`, though: every included row's wallet is the batch-level `cardLabel` (editable, defaults to a generic label or the model's guess). Approve creates only the currently-included rows and replies with the count + total + a "move to your Bancolombia pocket" reminder (no pocket entity is modeled — purely informational text). Fully reversible: undo deletes every created row. Unlike `propose_add_transaction`, a rule match here never auto-records — every batch still goes through the review card regardless of match. |
| `propose_create_counterparty_rule(matchType, matchValue, appCategoryName, wallet, direction?, autoRecord?, recurring?, expectedAmount?, notes?)` | `createCounterpartyRule()` | Create a counterparty rule (ADR-032). Call `get_categories` first — an unresolved `appCategoryName` **blocks** (never guesses): a wrong category here would silently misroute every future matching transaction. Supports undo (delete-the-created-rule). |
| `propose_update_counterparty_rule(ruleId, …fields)` | `updateCounterpartyRule()` | Update an existing rule's fields (ADR-032). Call `get_counterparty_rules` first to resolve `ruleId`. No undo (consistent with `propose_update_vault`). |
| `propose_delete_counterparty_rule(ruleId)` | `deleteCounterpartyRule()` | Delete a rule (ADR-032). Call `get_counterparty_rules` first to resolve `ruleId`. No undo. |
| `propose_undo_last()` | reverse of last approved action | Proposes reversal of the most recent approved conversational write. Reversible: createInstallment, markPayment, createLoan, recordPayment, createDebtor, createCard, accountAdjustment, transfer, addTransaction, addTransactionsBatch, createCounterpartyRule. Imports are NOT reversible. |
...
## 5g. Domain rules: Counterparty rules & auto-record (ADR-032, ADR-033)

**A transfer to a named account is a payment, not a self-transfer.** Colombian bank messages say "transferencia a la cuenta 617…" — that account is a destination the money is being paid TO (a person, a merchant, a purpose), never one of the user's own `SavingsAccount`s. Never propose `propose_transfer`/`propose_account_adjustment` for this; it's a `propose_add_transaction` like any other bank message.

**Extract the counterparty; matching is automatic.** When a bank message names a destination account, merchant, or sender, pass it to `propose_add_transaction` as `counterpartyAccount`/`counterpartyMerchant`/`counterpartySender` (plus `direction`), the same way you already extract amount/date/wallet/note. You do **not** need to call `get_counterparty_rules` yourself before proposing — the resolver consults the dictionary internally.

**Matched + autoRecord → recorded immediately, not a card.** If the counterparty matches a `CounterpartyRule` with `autoRecord: true` and the amount/date parsed with basic confidence, the transaction is created right away using the **rule's** `appCategoryId` and `wallet` — overriding whatever category you guessed and whatever account the message stated. The user is notified on Telegram (`✅ Registrado: …` with `[✏️ Editar] [↩︎ Deshacer]`) instead of seeing an approval card. Your tool_result confirms this happened; say so plainly ("recorded automatically per your rule for account 617…"), don't say "drafted for your approval" for this case.

**Unmatched, or matched-but-not-autoRecord → Phase 1 behavior, unchanged.** No rule, or a rule with `autoRecord: false`, or a low-confidence parse: falls through to the normal pre-filled editable card exactly as before (§5f). Nothing about the ordinary transaction flow changes when there's no confident match.

**Learning from corrections is a light nudge, not automatic.** After the user corrects or approves an unmatched transaction, they may see a follow-up suggestion offering to remember the mapping. If they reply affirmatively, build a `propose_create_counterparty_rule` from what was parsed (destination account for transfers, merchant for card purchases, sender for inbound) and let them adjust it before approving — this is the organic path; the settings page is the manual one.

---

## 5h. Domain rules: Card-screenshot batch ingestion (ADR-034)

Builds on §2b's Telegram image-input channel (Part 1) and §5g's counterparty-rule dictionary.

**When to use this tool.** A credit-card statement/screenshot (a LIST of card purchases, e.g. a banking app's transaction history) is `propose_add_transactions_batch`, not `propose_add_transaction` — never call the single-transaction tool once per row. A single bank notification (one purchase) is still `propose_add_transaction`, unchanged.

**Extraction is one-shot, no per-row questions.** Extract every visible row as `{ vendor, amount, date? }` (amount is always a positive magnitude — the tool negates it, since card purchases are always expenses) and mark rows that look crossed out / scratched as `scratched: true`, best-effort. Emit exactly ONE `propose_add_transactions_batch` call with the full list; never ask a clarifying question per row.

**Scratch-out detection is entirely the model's job.** The resolver only reads whatever `scratched` boolean the model sent (`included = !scratched`) — there is no server-side image re-analysis. If the model gets a scratch-out wrong, the user corrects it on the card (toggle button), not by re-sending the image.

**Category resolution per item:** a `CounterpartyRule` match by vendor (direction always EXPENSE — card purchases are never income) supplies the category; otherwise the same no-name fallback guess `propose_add_transaction`'s card already uses (first category, alphabetically). This is a DELIBERATE, narrower reuse than §5g's auto-record path — **a rule match here NEVER auto-records.** Every batch, matched or not, still goes through the review card; only the rule's `appCategoryId` is borrowed, never its `wallet`.

**Wallet = batch-level `cardLabel`, not the rule's wallet.** Every included row's wallet is set from the batch's `cardLabel` field (defaults to a generic label or the model's guess, editable on the card) — this is the opposite of §5g's single-transaction auto-record flow, where the rule's own wallet overrides the message. The difference is intentional: a card-screenshot batch is inherently "these purchases were all on this card," so the card label is the one wallet value that applies uniformly to the whole batch.

**Per-item and batch-level editing (fully DB-resolvable, ADR-031's design principle applied to a list):** every item is independently toggle-able (include/exclude) and category-editable; the card label is editable at the batch level. On Telegram, `bt:{idx}` toggles, `be:{idx}` opens a category picker for that row, `bs:{idx}:{optIdx}` applies a category, `bo` opens the card-label picker, `bc:{optIdx}` applies a card label — every one of these mutates `PendingProposal.params.batch` and re-renders the card without approving, answerable from a plain DB read (no agent re-run). On web, the same three mutations are exposed via `POST /api/proposals/batch-edit`.

**Approve creates only the currently-included rows.** Because toggles/edits mutate `params.batch` after the card was first shown, `execute()` reads the CURRENT batch state at approve time, not the original tool-call input. The reply reports the count and total of what was actually created, plus a reminder to move that total to the (un-modeled) Bancolombia pocket — purely informational text, no pocket entity exists. On Telegram, this summary becomes the **persistent edited-message text** (not just the ephemeral approve-toast) — see ADR-034's "Surfacing that message on Telegram" note.

**Fully reversible.** `propose_undo_last` (or "deshacer") removes every transaction the batch created, exactly like a single `propose_add_transaction` undo.
